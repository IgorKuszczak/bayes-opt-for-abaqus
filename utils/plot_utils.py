...
class Plot:
    ...
    def plot_evaluations_plt(self, best_objectives, mask):
        ...
        ax.scatter(x[mask], best_objectives.T[mask], c='b', edgecolor='black', marker='*', s=300,
                   label='Feasible solution', zorder=2)  # Feasible points

        if not np.all(mask == True):
            ax.scatter(x[~mask], best_objectives.T[~mask], c='r', marker='o', edgecolor='black', s=40,
                       label='Infeasible solution', zorder=1)  # Infeasible points
            plt.legend()
